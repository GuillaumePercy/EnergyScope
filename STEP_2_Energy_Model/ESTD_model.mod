...
set FLEXIBLE_TECH;
set STORAGE_TECH_ELEC;
set CONSTANT_TECH_ELEC;
...
## Parameters for Feedback loop : soft-link ESTD-Dispa-SET
param fbl_load_shedding_gwh >= 0, default 0; #GWh 
param fbl_load_shedding_duration >= 0, default 0; #h
param fbl_initial_DispatchPower >= 0, default 1; #  Total dispatchable power capacity+ import capacity initially installed [Gw]
param fbl_initial_Const_power >= 0, default 1;  # 
param fbl_LackOfPower >= 0, default 1; # determined by parametric analysis in Dispat-SET 0.875
...
# [Eq. 40]	
# Ensure minimum power capacity
subject to minimum_power_capacity:
	sum {j in FLEXIBLE_TECH, i in CONSTANT_TECH_ELEC} (F [j]+F [i] + import_capacity) >=  (fbl_initial_DispatchPower+ fbl_initial_Const_power) / fbl_LackOfPower;


# [Eq. 41]
var Sto_level_min >=0;
# Ensure minimum storage reserve 
subject to fix_security_reserve {tech in STORAGE_OF_END_USES_TYPES ["ELECTRICITY"], h in HOURS, td in TYPICAL_DAYS}:
	Sto_level_min <= F_t[ tech , h , td ]; 

# [Eq. 42]
#Force to have a 'reserve' large enough to supply the load_shedding	
subject to supply_capa_margin:
	(sum {tech in FLEXIBLE_TECH} F[tech]+ import_capacity - fbl_initial_DispatchPower) * fbl_load_shedding_duration + Sto_level_min >= fbl_load_shedding_gwh;
